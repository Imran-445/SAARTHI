...
import { Calculator, IndianRupee, ShieldCheck, Sparkles, Percent, HelpCircle, ArrowLeft, CheckCircle2 } from "lucide-react";

export default function CalculatorPage() {
  const location = useLocation();
  const [searchParams] = useSearchParams();

  // Read passed state from recommendations or details page
  const passedState = location.state || {};
  const passedSchemeName = passedState.schemeName || searchParams.get("schemeName");
  const passedLoan = Number(passedState.loanAmount || searchParams.get("loanAmount")) || 200000;
  const passedInterest = Number(passedState.interestRate || searchParams.get("interestRate")) || 8.5;
  const passedTenure = Number(passedState.tenureMonths || searchParams.get("tenureMonths")) || 36;
  const passedSubsidy = passedState.subsidyScheme || searchParams.get("subsidyScheme") || "pmegp_rural";

  const [loanAmount, setLoanAmount] = useState(passedLoan);
  const [interestRate, setInterestRate] = useState(passedInterest);
  const [tenureMonths, setTenureMonths] = useState(passedTenure);
  const [subsidyScheme, setSubsidyScheme] = useState(passedSubsidy);

  // Re-sync if navigation state updates
  useEffect(() => {
    if (passedState.loanAmount) setLoanAmount(Number(passedState.loanAmount));
    if (passedState.interestRate) setInterestRate(Number(passedState.interestRate));
    if (passedState.tenureMonths) setTenureMonths(Number(passedState.tenureMonths));
    if (passedState.subsidyScheme) setSubsidyScheme(passedState.subsidyScheme);
  }, [location.state]);
    ...
    <div className="py-10 bg-slate-50 min-h-screen text-left">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="bg-white rounded-3xl p-6 sm:p-8 border border-slate-200 shadow-soft mb-8">
          ...
          {/* Contextual Scheme Pre-fill Banner */}
          {passedSchemeName && (
            <div className="mt-5 p-3.5 rounded-2xl bg-emerald-50 border border-emerald-200 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 text-xs">
              <div className="flex items-center space-x-2 text-emerald-950 font-bold">
                ...
                <span>Pre-filled parameters for: <strong>{passedSchemeName}</strong></span>
              </div>
              <Link
                to={location.state?.returnPath || "/recommended-scheme"}
